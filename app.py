...
# Conexão Simples (para banco e download)
supabase = create_client(SUPABASE_URL, SUPABASE_KEY)

@app.route('/')
def home():
    return "🤖 Robô V2 (TUS Upload) Ativo."
    ...
    try:
        ...
        with open(nome_temp, "wb") as arquivo_final:
            for i, p in enumerate(partes):
                print(f"⬇️ Baixando parte {i+1}/{len(partes)}...")
                data = supabase.storage.from_("gravacoes").download(f"{caminho_base}/{p['name']}")
                arquivo_final.write(data)

        # C. UPLOAD VIA TUS (A Mágica para arquivos gigantes)
        print("⬆️ Iniciando Upload TUS (Chunked)...")
        path_destino = f"{caminho_base}/video_completo_render.webm"
        
        # Configura o Cliente TUS do Supabase
        tus_url = f"{SUPABASE_URL}/storage/v1/upload/resumable"
        my_client = client.TusClient(url=tus_url, headers={
        ...
        uploader = my_client.uploader(
            file_path=nome_temp,
            chunk_size=6 * 1024 * 1024, # 6MB por pedaço
            metadata={
        ...
        uploader.upload() # Isso envia pedacinho por pedacinho
        ...
        supabase.table("reuniao_processing_queue").update({"status": "CONCLUIDO", "log_text": "Sucesso TUS"}).eq("id", job_id).execute()
